...
module.exports = ({ mongoose, Pool }) => {
  ...
  const authenticate = async (instance_id) => {
    ...
    if (!user) {
      return Promise.reject(
        Object.assign(new Error("User was not found"), { statusCode: 404 })
      );
    }
    return user;
  };
  const checkIfUserExists = async (instance_id) => {
    ...
    if (user) {
      return Promise.reject(
        Object.assign(
          new Error(`User instance_id: ${instance_id} already taken`),
          { statusCode: 409 }
        )
      );
    }
  ...
  return {
    ...
    mePostDatapoint: async (req, res, next) => {
      try {
        ...
        const datapoint = await DatapointModel.create({
          instance_id: instance_id || body.instance_id,
          ...body,
        });
    ...
    meBatchPostDatapoint: async (req, res, next) => {
      try {
        ...
        const datapoint = await DatapointModel.insertMany(
          new_datapoints.datapoints.map((d) => ({ ...d, instance_id }))
        );
